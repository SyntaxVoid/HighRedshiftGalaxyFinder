...
if __name__ == '__main__':
    ...
    if SELECT_ME:
        RunSelections.run("masterNONE.cat","SelectedObjects/Mine/NONE/",MASTER_COL_DICT,False)
        RunSelections.run("masterRMS.cat","SelectedObjects/Mine/RMS/",MASTER_COL_DICT,True)
    if COLOR_COLOR_ME:
        ColorColorME.run("SelectedObjects/Mine/NONE/",rms = False)
        ColorColorME.run("SelectedObjects/Mine/RMS/",rms = True)
    if SELECT_C:
        RunSelections.run("Candels_Catalog/CANDELS.GOODSS.F160W.v1_1.photom.cat","SelectedObjects/Candels/",MASTER_CANDELS_DICT,False)
        #
        # print("\n" + "="*80)
